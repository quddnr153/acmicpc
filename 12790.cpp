...
1 초	128 MB	1722	888	850	55.519%
문제
...
전투력 계산은 굉장히 간단한 로직이지만 게임 곳곳에 쓰이는 만큼,
Being은 다른 프로그래머로 하여금 이 로직을 구현하여 자신의 것과 교차검증하고자 한다.
그리하여 당신은 이 전투력을 구현하는 작업을 맡게 되었다.

능력치는 모두 네 종류 존재한다.
이는 각각 HP, MP, 공격력, 방어력이다.
...
처음 네 개의 정수는 각각 캐릭터의 기본 HP, MP, 공격력, 방어력이다.
이 값들은 1 이상 999 이하로 주어진다.
...
void test()
{
	int basicA[4], additionalA[4], finalA = 0;
	for(int i = 0; i < 4; i++)
		cin >> basicA[i];
	for(int i = 0; i < 4; i++)
		cin >> additionalA[i];
	...
	for(int i = 0; i < 4; i++)
		basicA[i] += additionalA[i];

	if(basicA[0] < 1) basicA[0] = 1;
	if(basicA[1] < 1) basicA[1] = 1;
	if(basicA[2] < 0) basicA[2] = 0;
	
	finalA = basicA[0] + 5 * basicA[1] + 2 * basicA[2] + 2 * basicA[3];
